...
## I use full gradient descent in this case (probably also on the real data)
## in FGD, the updating becomes different: for the individual factors, they are equally updated instead of being proportional to the amount of samples





## this script seems good; several factors:
##	1. use iterative approach
##	2. initialization is important (try to not fit the training data too well, such that training will further test set performance)
##	3. use full GD for now
...
with tf.device("/cpu:0"):
	...
	result_exp = tf.matmul(m_factor, beta2_slope) + beta2_intercept
	...
	for i in xrange(10000):
		print "iter#", i,
		## training
		sess.run(training_step1, feed_dict={x: X, y: Y_spread[list_index_sample_train], y_index: list_index_sample_train})
		sess.run(training_step2, feed_dict={x: X, y: Y_spread[list_index_sample_train], y_index: list_index_sample_train})
		sess.run(training_step3, feed_dict={x: X, y: Y_spread[list_index_sample_train], y_index: list_index_sample_train})
